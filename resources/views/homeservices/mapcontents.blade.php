...
<script type="text/javascript">
 function initialize() {
	...
	var image1 = "{{URL::asset('public/images/icon01.png')}}";
	var image2 = "{{URL::asset('public/images/icon-2.png')}}";
	var image3 = "{{URL::asset('public/images/icon03.png')}}";
	var image4 ="{{URL::asset('public/images/icon01.png')}}";
	var image5 = "{{URL::asset('public/images/icon-2.png')}}";
	
	
	
	
 //   var locations = [
	// @foreach($servicesMap as $article)
	//   [{{$article->lat}}, {{$article->lon}}], 
	// @endforeach
	// ];
	
	@foreach($servicesMap as $listing)
	    		
				marker{{ $listing->id }} = new google.maps.Marker({
		        position: {lat: {{$listing->lat}}, lng: {{$listing->lon}}},
				map: map,
		        visible: true,
				icon: image{{ $listing->id }}
		        });
